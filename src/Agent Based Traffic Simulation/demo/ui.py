...
def run_pygame(model, width=800, height=800, agent_radius=3):
    ...
    scale_x = width / model.space.x_max
    scale_y = height / model.space.y_max
    ...
    while running:
        ...
        for i, a in enumerate(model.agents):
            ...
            py = int(a.pos[1] * scale_y)

            current_turn = a.current_turn
            turn_reset_len = a.turn_reset_len

            color = tuple(map(int,(255*(1 - max(0, 2*min(current_turn/turn_reset_len, 1) - 1)), 255*min(1, 2*min(current_turn/turn_reset_len, 1)), 0)))

            # Draw agent
            pygame.draw.circle(screen, color, (px, py), agent_radius)
